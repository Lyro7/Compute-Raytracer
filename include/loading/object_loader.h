...
public:
	 ...
	 *
	 * @param[in] path Path to the OBJ file on disk.
	 * 
	 * @return A Mesh containing all loaded triangles. If loading fails, an empty
	 *         Mesh is returned.
	 *
	 * @note OBJ files must contain vertex normals.
	 ...
	 *
	 * @param[in] objBytes Byte buffer containing the OBJ file contents.
	 * @param[in] virtualName Virtual name used for error reporting and debugging.
	 * @param[in] mtlBytes Optional byte buffer containing the MTL file contents.
	 *
	 * @return A Mesh containing all loaded triangles. If parsing fails or the
	 *         OBJ data is empty, an empty Mesh is returned.
	 *
	 * @note OBJ data must contain vertex normals.
	 * @note Texture coordinates are currently ignored.
	 * @note This function is useful for loading meshes from archives, network
	 *       streams, or embedded resources.
	 */
	static Mesh loadMeshFromMemory(const std::vector<uint8_t> &objBytes, 
